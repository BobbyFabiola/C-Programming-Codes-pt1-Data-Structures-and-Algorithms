...
{
    //! Write your code
    // studLL *temp, newNode = (studLL)malloc(sizeof(studNode));
    // if(newNode != NULL){
    //     for(temp = SL ; 
    //     *temp != NULL && (strcmp((*temp)->stud.idNum, S.idNum) < 0) ; 
    //     temp = &(*temp)->next){}
    ...
    //         *temp = newNode;
    //         return 1;
    //     }else if(strcmp((*temp)->stud.idNum, S.idNum) == 0){
...
void convertToDCISMDict(dcismDict D, arrListStud SL)
{
     ...
     for(ndx = 0 ; ndx < SL.numStuds ; ndx++){ //goes through all students
        ...
        int ylKey = yearLevelHash(SL.studs[ndx].info);  //gets year
